  ...
  export const fetchData=async (query)=>{
    const link=`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(query)}&format=json&limit=1`;
    ...
    throw new Error(`NETWORK_ERROR ${response.status} ${response.statusText}`);
  }
    }
...
async function getDestance(originLon,originLat,desLon,desLat){
    const response=await fetch(`https://api.openrouteservice.org/v2/directions/driving-car?api_key=eyJvcmciOiI1YjNjZTM1OTc4NTExMTAwMDFjZjYyNDgiLCJpZCI6Ijg2NDc0YjE3YmE3NDRhMGRhMDEzN2I3NzczZDA0ZmI3IiwiaCI6Im11cm11cjY0In0=&start=${originLon},${originLat}&end=${desLon},${desLat}`);
    if(!response.ok){
     if (response.status === 404) {
    throw new Error(`NOT_FOUND ${response.status} ${response.statusText}`);
  } else if (response.status === 500) {
 ...
 export  const getCoordinates= async ()=>{
    ...
    const [originData,destinationDate]=await Promise.all([fetchData(inputLocation),fetchData(inputDestination)]);
    const {lat:originLat,lon:originLon}=originData;
    const{lat:desLat,lon:desLon}=destinationDate;
    const {distanceKmMeters,durationMin,coordinates}=await getDestance(originLon,originLat,desLon,desLat);
    InitResultsPage(originLat,originLon,desLat,desLon,distanceKmMeters,durationMin,coordinates,inputLocation,inputDestination);
    }catch(error){
        console.error(error);
...
export const getDrivers=async(distance,duration,originLon,originLat)=>{
    const baseFare=3;
        ...
        const result=await getDriversToCustomerMatrix(originLon,originLat,drivers);
        console.log(result);
...
async function fetchDrivers(){
    ...
    if(!response.ok){
        throw new Error(`FetchDriver error ${response.status} ${response.statusText}`);
...
async function getDriversToCustomerMatrix(originLon,originLat,drivers){
    const apiKey='eyJvcmciOiI1YjNjZTM1OTc4NTExMTAwMDFjZjYyNDgiLCJpZCI6Ijg2NDc0YjE3YmE3NDRhMGRhMDEzN2I3NzczZDA0ZmI3IiwiaCI6Im11cm11cjY0In0=';
    const locations=[
        [originLon,originLat],
...
export const getSuggestion=async (query)=>{
    const url=`https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(query)}&format=json&limit=5&countrycodes=nl`;
...
/*async function renderSuggestions(suggestions){
        const suggestionsList=document.getElementById('pickupSuggestions');
